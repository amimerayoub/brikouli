...
export function MapDiscovery() {
  ...
  const gigs = nearby.data?.success ? nearby.data.data : [];
  ...
  return <section className={`map-discovery map-mode-${mode}`}>
    ...
    <BottomSheet open={Boolean(selectedGig)} onClose={() => setSelectedGigId(null)} title="تفاصيل الفرصة"><>{selectedGig && <MarkerPopup gig={selectedGig} onClose={() => setSelectedGigId(null)} />}</></BottomSheet>
  </section>;
}

...
export default function Home() {
  ...
  const gigs = gigsQuery.data?.success ? gigsQuery.data.data : [];
  const savedIds = new Set(favoriteIdsQuery.data?.success ? favoriteIdsQuery.data.data : []);
  const recommended = [...gigs].sort((left, right) => Number(right.urgent) - Number(left.urgent) || right.payment - left.payment).slice(0, 2);
  const recent = gigs.slice(0, 5);
  const showSearchFeedback = Boolean(deferredSearch || category) && !gigsQuery.isLoading && !gigsQuery.isError && gigs.length === 0;
  ...
  return <AppShell><section className="home-welcome job-home-welcome"><motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .26 }}><p className="welcome-eyebrow"><MapPinned size={15} /> الفرص القريبة تبدأ من هنا</p><h1>أهلاً بك في <em>بريكولي</em></h1><p>ابحث، احفظ، وتقدّم إلى فرصة مناسبة لوقتك في الحي.</p></motion.div><div className="job-home-search-stack"><JobSeekerSearch value={search} onChange={setSearch} placeholder="ابحث عن فرصة أو متجر أو حي" /><JobSeekerCategories categories={categories} value={category} onChange={setCategory} /></div><Link href="/explore" className="home-explore-cta">استكشف على الخريطة <ArrowLeft size={17} /></Link></section>
    ...
    {gigsQuery.isLoading && <section className="phase-section"><PageHeading eyebrow="يتم التحديث" title="فرص تناسبك" /><div className="home-job-grid"><JobSeekerCardSkeleton /><JobSeekerCardSkeleton /></div></section>}
    {gigsQuery.isError || (gigsQuery.data && !gigsQuery.data.success) ? <section className="phase-section"><JobSeekerLoadError onRetry={() => void gigsQuery.refetch()} /></section> : null}
    {showSearchFeedback ? <section className="phase-section"><NoSearchResults query={search || category || "اختيارك"} /></section> : null}
    {!gigsQuery.isLoading && !gigsQuery.isError && !showSearchFeedback && gigs.length === 0 ? <section className="phase-section"><NoJobsFound /></section> : null}
    {recommended.length > 0 && <section className="phase-section"><PageHeading eyebrow="تابع من حيث توقفت" title={search || category ? "نتائج تناسب بحثك" : "فرص قد تناسبك"} action={<Link className="section-link" href="/explore">عرض الكل <ArrowLeft size={15} /></Link>} /><div className="home-job-grid">{recommended.map(gig => <JobSeekerGigCard gig={gig} isSaved={savedIds.has(gig.id)} onToggleSave={toggleSave} key={gig.id} />)}</div></section>}

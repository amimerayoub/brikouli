import type { ApiResult, Gig, NearbyGig } from "@shared/brikouli.types";
import { gigSchema, nearbyGigQuerySchema } from "../schemas/domain";
...
function mapNearbyGig(row: Record<string, unknown>): NearbyGig { return { ...mapGig(row), employerName: String(row.employer_name ?? "صاحب عمل محلي"), distanceMeters: Number(row.distance_meters) }; }
...
export async function getNearbyGigs(input: unknown): Promise<ApiResult<NearbyGig[]>> {
  ...
  return { success: true, data: (data ?? []).map((row: unknown) => mapNearbyGig(row as Record<string, unknown>)) };
}

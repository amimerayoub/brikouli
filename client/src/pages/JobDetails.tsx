...
export function ApplicationSuccessPanel({ onClose }: { onClose: () => void }) {
  return <motion.div className="application-success" initial={{ opacity: 0, y: 8, scale: .97 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: .24 }}><span>✓</span><h2>تم إرسال طلبك</h2><p>ستظهر أي تحديثات من صاحب المنشور في صفحة طلباتك.</p><Link href="/applications" className="app-button app-button-primary" onClick={onClose}>متابعة طلباتي</Link></motion.div>;
}
